...
- No collection or transmission of player data beyond the documented local recorder, scanner, harvest, reports and persistence features. No new outbound chat, invitations, queries, or network-like features without an explicit player-facing design and approval. Do not put player names or raw SavedVariables into PRs or releases.
...
- `Crowd.lua`: optional crowd banner can run `/who` and set who-result routing automatically; its Invite button can invite several nearby players on one click.
