...
- Quest items you click now have a button on the step's row: the totem you plant at a well, the widget you use at a pool, the bane you use on a mob, or an item in your bags that starts the quest. It shows once the item is in your bags and the objective is still open, and goes away when it is done. Items with no Use effect (letters, deeds) get no button. For WoW Forever's own quests, the item the quest log itself attaches is used.
- The target-next-quest-mob key also uses the step's quest item: it targets the mob first and then uses the item, so a use-on-a-mob item lands on the mob it just picked. On a step with no mob to target, the key just uses the item. Turn it off in Options (Quest mobs) or with `/fg skull item off`.
- Profession steps (the cooking quests) were shown to characters without the profession: the check read skills the Classic way, which WoW Forever does not offer, and showed every step when it found none. It now reads Forever's own skill list (`C_SkillInfo`), matched by skill ID, so it also works on non-English clients.
- Skyborne characters were offered race-only quests (Human-only, Tauren-only, and the other faction's quests): the race check did not know the Skyborne race and let every quest through. A Skyborne now takes what every race of its faction can take, and nothing race-specific. On the Horde Skyborne route this drops A Bundle of Hides and Meats to Orgrimmar.
...
- Mob tooltips list just the count and the objective ("0/8 Bear Fur"): no quest title, and the count only once.
